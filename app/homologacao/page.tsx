  ...
  const loadHomologationProducts = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const productsWithStatus = (data.data?.data || []).map((p: any) => ({
          ...p,
          id: String(p.id),
  ...
  const runHomologationTests = async () => {
    ...
    try {
      ...
      if (createResponse.ok && createData.data?.id) {
        testProductId = String(createData.data.id)
        setTestResults((prev) => ({
          ...prev,
            ...
            >
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="descricao_curta" className={`${isWakanda ? "text-green-100" : "text-white"}`}>
                      Descrição Curta
                    </Label>
                    <Textarea
                      id="descricao_curta"
